...
export default class GameOverScene extends Phaser.Scene {
    ...
    create(data) {
        const { score = 0 } = data;
        const goldAmount = Math.trunc(score * 1 * data.scene.levels[data.scene.registry.get('currentLevel')].levelConfigs.addGoldAndGemsCoefficient);
        const gemAmount = Math.trunc(score * 0.3 * data.scene.levels[data.scene.registry.get('currentLevel')].levelConfigs.addGoldAndGemsCoefficient);
